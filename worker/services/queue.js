...
function nextWorkdayAt1519UK(fromMs = Date.now()) {
  const d = new Date(fromMs);
  d.setUTCDate(d.getUTCDate() + 1);
  d.setUTCHours(13, 19, 0, 0); // 13:19 UTC
...
export async function enqueueFree(env, {
  ...
  // Tier3 gets stage 1 only (free analysis) — no follow-up pressure.
  ...
  // Stage 2 and 3 are scheduled relative to the previous stage,
  // so they can never become due before an earlier stage.
