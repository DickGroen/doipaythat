// ── Shared frontend helpers for DoIPayThis ────────────────────────────────────
...
export function validateFile(file) {
  ...
  if (file.size > MAX_FILE_SIZE) return `File too large (max 8 MB, yours is ${(file.size / 1024 / 1024).toFixed(1)} MB)`;
  const ext = '.' + file.name.split('.').pop().toLowerCase();
  if (!ALLOWED_EXTENSIONS.includes(ext)) return `File type not allowed. Use PDF, JPG or PNG.`;
  return null;
}
