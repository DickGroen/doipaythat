...
function getFallbackTeaser(risk) {
  if (risk === 'high') {
    return 'There are points in this charge that may be worth checking carefully before you pay \u2014 service charges and deductions are not always fully itemised or supported.';
  }

  if (risk === 'medium') {
    return 'There may be aspects of this charge worth checking before you pay \u2014 such as how the amount is broken down and whether it is properly supported.';
  }

  return 'The charge appears relatively straightforward, though it may still be worth checking the main points before paying.';
}
    ...
    if (card) {
      card.innerHTML = `
        <div class="success-screen">
          ...
          <p>We will review your housing document and send you the full review and response letter by email to <strong>${esc(email)}</strong>.</p>
