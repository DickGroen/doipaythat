...
// ── File input change listener (works on all devices) ─────────────────────────
const fileInputEl = document.getElementById('gratis-file-input');
if (fileInputEl) {
  fileInputEl.addEventListener('change', function() {
    if (this.files?.[0]) {
      window.handleGratisFileSelect(this);
    }
  });
}
...
// Listen on input, change AND blur for iOS compatibility
...
window.startGratisUpload = startGratisUpload;
...
window.goToStripe = async function() {
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = ctaText(TYPE); }
...
async function doSubmit() {
  ...
  try {
    ...
    if (card) {
      card.innerHTML = `<div class="success-screen"><div class="success-screen__icon">✓</div><h2>Upload successful</h2><p>We will analyse your letter and send you the full review and response draft by email to <strong>${esc(email)}</strong>.</p><p style="font-size:.82rem;color:var(--muted);">Please also check your spam folder.</p></div>`;
    }
  } catch (err) {
