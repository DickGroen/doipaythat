...
window.handleGratisFileSelect = function(input) {
  ...
  if (err) {
    if (status) { status.className = 'optie-status optie-status--error'; status.textContent = err; }
    return;
  }
  const zone = document.getElementById('gratis-upload-zone');
  if (zone) {
    zone.innerHTML = `<div class="upload-label" style="color:var(--green);">\u2713 ${esc(freeFile.name)}</div><div class="upload-hint">${formatFileSize(freeFile.size)}</div>`;
  }
...
function checkFreeReady() {
  ...
  btn.disabled = !ready;
  if (hint) {
    if (email.length > 3 && !emailOk) { hint.textContent = 'Please enter a valid email address.'; hint.style.display = 'block'; }
...
window.startGratisUpload = async function() {
  ...
  if (!freeFile || !name || !emailOk) {
    if (status) { status.className = 'optie-status optie-status--error'; status.textContent = 'Please enter your name, a valid email address and select a file.'; }
    return;
  }
  if (btn) { btn.disabled = true; btn.textContent = 'Checking your document\u2026'; }
  try {
    const data = await submitFree({ file: freeFile, name, email, type: TYPE,
      onStatus: (kind, msg) => { if (!status) return; status.className = `optie-status optie-status--${kind}`; status.textContent = msg; }
    });
    const triage = normalizeTriage(data.triage || {});
    stripeLink = data.stripeLink || data.teaser?.stripeLink || triage.stripeLink || stripeLink;
    ...
    if (status) { status.className = 'optie-status optie-status--success'; status.textContent = 'Your first check is ready.'; }
...
function normalizeTriage(triage) {
  ...
  return { ...triage, risk, teaser: triage.teaser || getFallbackTeaser(risk) };
}

function getFallbackTeaser(risk) {
  if (risk === 'high') return 'There may be several aspects of this bill worth checking carefully before payment.';
  if (risk === 'medium') return 'There may be aspects of this bill worth a closer look before you pay.';
  return 'At first glance the bill appears relatively standard. It may still be worth checking the main points before paying.';
}
...
window.goToStripe = async function() {
  ...
  const name  = document.getElementById('gratis-name')?.value.trim()  || '';
  const email = document.getElementById('gratis-email')?.value.trim() || '';
  if (!name || !email) { openModal('modal'); return; }
  ...
  try {
    const res = await fetch('/api/create-checkout', { method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: TYPE, name, email, freeSessionId: stripeLink || '' }) });
    const data = await res.json();
    if (data.ok && data.url) { window.location.href = data.url; }
    else { throw new Error(data.error || 'Checkout failed'); }
  } catch (err) {
    if (btn) { btn.disabled = false; btn.textContent = `Review the bill in full \u2014 ${CURRENCY}${PRICE} \u2192`; }
...
if (document.getElementById('submit-btn')) {
  ...
  fileInput?.addEventListener('change', () => { if (fileInput.files?.[0]) updateSelectedFile(fileInput.files[0]); });
  ...
  uploadPanel?.addEventListener('drop', e => { e.preventDefault(); uploadPanel.classList.remove('drag-over'); if (e.dataTransfer.files?.[0]) { fileInput.files = e.dataTransfer.files; updateSelectedFile(e.dataTransfer.files[0]); } });
...
function updateSelectedFile(file) {
  ...
  if (btn) { btn.disabled = false; btn.textContent = 'Upload and start analysis'; }
}
...
function clearFile() {
  ...
  const btn = document.getElementById('submit-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'Choose a file first'; }
}
...
async function doSubmit() {
  ...
  if (!name || !emailOk || !file) { showStatus('Please fill in all fields correctly and select a file.', 'error'); return; }
  const btn = document.getElementById('submit-btn');
  if (btn) { btn.disabled = true; btn.textContent = 'Uploading\u2026'; }
  try {
    await submitPaid({ file, name, email, type: TYPE, sessionId: params.get('session_id'), onStatus: showStatus });
    const fallback = document.getElementById('thankyou-app');
    if (fallback) fallback.classList.add('hidden');
    const success = document.getElementById('success-screen');
    if (success) { success.classList.remove('hidden'); return; }
  } catch (err) {
